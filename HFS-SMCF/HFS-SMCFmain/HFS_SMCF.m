function [feature_slct,W] = HFS_SMCF(X, Y, tree, lambda, treecor,sibcor,alpha,beta,ga,flag,labelsimi)
q=1;ma=[];mi=[];
...
%            W{internalNodes(j)} = inv(XX{internalNodes(j)} + lambda * D{internalNodes(j)}) * (XY{internalNodes(j)}-alpha*W_current1-beta*W_current2-0.35*W_current3);
          W{internalNodes(j)} = inv(XX{internalNodes(j)} + lambda * D{internalNodes(j)}) * (XY{internalNodes(j)})-alpha*W_current1-beta*W_current2-ga*W_current3;
